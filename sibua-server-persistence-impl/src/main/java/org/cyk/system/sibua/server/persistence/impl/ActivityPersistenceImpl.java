...
public class ActivityPersistenceImpl extends AbstractPersistenceEntityImpl<Activity> implements ActivityPersistence,ReadActivityBySections,ReadActivityByPrograms,ReadActivityByActions,ReadActivityByAdministrativeUnits,ReadActivityByUsers,Serializable {
	...
	,readByFiltersCodesLike,readByUsersIdentifiers,readWhereCodeOrNameContainsAndSectionCodeLikes,readWhereIsGestionnaireOrBeneficiaireByAdministrativeUnitsCodes;
	...
	protected void __listenPostConstructPersistenceQueries__() {
		...
		addQueryCollectInstances(readWhereIsGestionnaireOrBeneficiaireByAdministrativeUnitsCodes, "SELECT activity FROM Activity activity "
				+ "WHERE "
				+ "EXISTS (SELECT administrativeUnitActivity FROM AdministrativeUnitActivity administrativeUnitActivity WHERE administrativeUnitActivity.activity = activity "
				+ "AND administrativeUnitActivity.administrativeUnit.code IN :administrativeUnitsCodes) "
				+ "OR "
				+ "EXISTS (SELECT administrativeUnitActivity FROM AdministrativeUnitActivity administrativeUnitActivity WHERE administrativeUnitActivity.activity = activity "
				+ "AND administrativeUnitActivity.administrativeUnitBeneficiaire.code IN :administrativeUnitsCodes) "
				+ "ORDER BY activity.code ASC");
				
		addQueryCollectInstances(readWhereCodeOrNameContainsAndSectionCodeLikes, "SELECT activity FROM Activity activity WHERE activity.code LIKE :code OR activity.name LIKE :name AND activity.action.program.section.code LIKE :sectionCode ORDER BY activity.code ASC");
		...
		addQueryCollectInstances(readByFiltersCodesLike, "SELECT activity FROM Activity activity "
				...
				+ "AND (:anyAdministrativeUnit = true OR EXISTS (SELECT administrativeUnitActivity FROM AdministrativeUnitActivity administrativeUnitActivity"
				+ " WHERE administrativeUnitActivity.activity = activity AND "
				+ "administrativeUnitActivity.administrativeUnit.code LIKE :administrativeUnitCode "
				+ "OR administrativeUnitActivity.administrativeUnit.name LIKE LOWER(:administrativeUnitName) "
				+ "OR administrativeUnitActivity.administrativeUnitBeneficiaire.code LIKE :administrativeUnitBeneficiaireCode "
				+ "OR administrativeUnitActivity.administrativeUnitBeneficiaire.name LIKE LOWER(:administrativeUnitBeneficiaireName) "
				+ ")) "
				+ AND_PROGRAM_BUDGET_CATEGORY_CODE_IS_1_OR_3_OR_7
				//+ "AND NOT EXISTS (SELECT administrativeUnitActivity FROM AdministrativeUnitActivity administrativeUnitActivity WHERE administrativeUnitActivity.activity = activity) "
				+ "ORDER BY activity.code ASC");
	...
	protected void __listenExecuteReadAfterSetFieldValue__(Activity activity, Field field, Properties properties) {
		...
		} else if(field.getName().equals(Activity.FIELD_NUMBER_OF_COST_UNITS)) {
			activity.setNumberOfCostUnits(__inject__(ActivityCostUnitPersistence.class).count(new Properties()
					.setQueryIdentifier(ActivityCostUnitPersistence.COUNT_BY_ACTIVITIES_CODES).setQueryFilters(new Filter().addField(ActivityCostUnit.FIELD_ACTIVITY
							, List.of(activity.getCode())))).intValue());
	...
	*/
	...
	protected String __getQueryIdentifier__(Class<?> klass, Properties properties, Object... objects) {
		...
		if(PersistenceFunctionReader.class.equals(klass)) {
			if(Boolean.TRUE.equals(__isFilterByKeys__(properties, Activity.FIELD_ADMINISTRATIVE_UNIT_GESTIONNAIRE_OR_BENEFICIAIRE)))
	...
	protected Object[] __getQueryParameters__(QueryContext queryContext, Properties properties,Object... objects) {
		...
		if(queryContext.getQuery().isIdentifierEqualsToOrQueryDerivedFromQueryIdentifierEqualsTo(readWhereIsGestionnaireOrBeneficiaireByAdministrativeUnitsCodes)) {
			if(ArrayHelper.isEmpty(objects))
				objects = new Object[] {queryContext.getFilterByKeysValue(Activity.FIELD_ADMINISTRATIVE_UNIT_GESTIONNAIRE_OR_BENEFICIAIRE)};
			return new Object[]{"administrativeUnitsCodes",objects[0]};
		}
		...
		if(queryContext.getQuery().isIdentifierEqualsToOrQueryDerivedFromQueryIdentifierEqualsTo(readByFiltersCodesLike)) {
			if(ArrayHelper.isEmpty(objects)) {
				objects = new Object[] {queryContext.getStringLike(Activity.FIELD_CODE),queryContext.getStringLike(Activity.FIELD_NAME)
						...
						,queryContext.getStringLike(Activity.FIELD_ACTION),queryContext.getStringLike(Activity.FIELD_ADMINISTRATIVE_UNIT)
			...
			objects = new Object[]{Activity.FIELD_CODE,objects[0],Activity.FIELD_NAME,objects[1],"sectionCode",objects[2],"programCode",objects[3],"actionCode",objects[4]
					,"anyAdministrativeUnit",Boolean.TRUE,"administrativeUnitCode",objects[5],"administrativeUnitName",objects[5]
							,"administrativeUnitBeneficiaireCode",objects[6],"administrativeUnitBeneficiaireName",objects[6]};
			return objects;
		}
